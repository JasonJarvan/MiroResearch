...
- `MIROMIND_API_KEY` is resolved in order: **process env → `.env` in the skill
  root** (next to `scripts/`). If neither has it and you're on an interactive
  terminal, the script **prompts once and saves it to `.env`** (chmod 600).
  In an agent/non-interactive shell it exits cleanly (code 3) instead of
  hanging — so if a run fails with "MIROMIND_API_KEY is not set", either add it
  to the skill-root `.env` (`MIROMIND_API_KEY=...`) or ask the user for it.
- `MIROMIND_BASE_URL` optional (env or `.env`), default `https://api.miromind.ai/v1`.
