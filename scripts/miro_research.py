...
Auth: MIROMIND_API_KEY, resolved in order — process environment, then a `.env`
file in the skill root (the dir containing this scripts/ folder). If still
missing AND a TTY is attached, the script prompts once and saves the key to
that `.env` (chmod 600). In a non-interactive (agent) context it exits cleanly
with code 3 instead of hanging on input.
Base URL: MIROMIND_BASE_URL (env or .env; default https://api.miromind.ai/v1).
...
def _env_path() -> str:
    """Path to the skill-root `.env`. Uses realpath so the single deployed
    `.env` is found regardless of which agent's symlink invoked this script."""
    script_dir = os.path.dirname(os.path.realpath(__file__))
    skill_root = os.path.dirname(script_dir)  # parent of scripts/
    return os.path.join(skill_root, ".env")


def _load_env_file(path: str) -> dict[str, str]:
    """Parse a minimal KEY=VALUE `.env` (comments/blank lines ignored).
    Missing file -> {}; unreadable file -> {} with a warning."""
    values: dict[str, str] = {}
    try:
...
def _save_key_to_env(path: str, api_key: str) -> None:
    """Write/replace the MIROMIND_API_KEY line in `.env`, preserving other
    lines, then chmod 600 (the file holds a secret). Atomic via os.replace."""
    try:
        with open(path, "r", encoding="utf-8") as f:
    ...
    out, replaced = [], False
    for line in lines:
        if line.strip().split("=", 1)[0].strip() == "MIROMIND_API_KEY":
            out.append(f"MIROMIND_API_KEY={api_key}")
            replaced = True
        else:
            out.append(line)
    if not replaced:
...
def _prompt_for_key(path: str) -> str:
    """Interactively ask for the key and save it to `.env`. Returns "" when no
    TTY is attached (agent/non-interactive context) so callers can fail fast
    instead of hanging on input."""
    if not sys.stdin.isatty():
        return ""
    sys.stderr.write(f"MIROMIND_API_KEY not found; it will be saved to {path}\n")
    try:
        api_key = getpass.getpass("Enter MIROMIND_API_KEY: ").strip()
    except (EOFError, KeyboardInterrupt):
        sys.stderr.write("\n")
        return ""
    if api_key:
        _save_key_to_env(path, api_key)
        sys.stderr.write(f"saved MIROMIND_API_KEY to {path}\n")
    return api_key
...
def _config() -> tuple[str, str]:
    ...
    api_key = (os.environ.get("MIROMIND_API_KEY")
               or file_env.get("MIROMIND_API_KEY") or "").strip()
    if not api_key:
        api_key = _prompt_for_key(env_path)
    if not api_key:
        sys.stderr.write(
            "error: MIROMIND_API_KEY is not set.\n"
            f"       add it to {env_path} (MIROMIND_API_KEY=...), export it,\n"
            "       or run this command in an interactive terminal to enter it once.\n"
        )
        raise SystemExit(3)
    base_url = (os.environ.get("MIROMIND_BASE_URL")
                or file_env.get("MIROMIND_BASE_URL") or DEFAULT_BASE_URL).rstrip("/")
