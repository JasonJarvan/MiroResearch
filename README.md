...
|---------|----------|---------|-------|
| `MIROMIND_API_KEY` | yes | — | Resolved from process env, else a `.env` in the skill root. If missing on an interactive terminal, the script prompts once and writes it to `.env` (chmod 600). Non-interactive runs exit cleanly (code 3) rather than hang. |
| `MIROMIND_BASE_URL` | no | `https://api.miromind.ai/v1` | Env or `.env`. Override only if your gateway differs. |

Config lookup for the key: **process env → skill-root `.env`**. Copy
`.env.example` to `.env` and fill it in, or just run the script once
interactively to be prompted. `.env` is gitignored — never commit your key.
